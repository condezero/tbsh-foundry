export default class TBSHCombat extends Combat {
    /**
     * Called to advance a combat to the next round.
     */
    async nextRound() {
        let roundIndex   = this.round;
        let roundHistory = (this.getFlag("tbsh", "roundHistory") || []);
        ...
        for(let i = 0; i < results.length; i++) {
            if(results[i].roll) {
                ...
                let messageData = foundry.utils.mergeObject({speaker: {scene: this.scene.id,
                                                             ...
                                                             flavor: game.i18n.format("tbsh.initiative.roll"),
    ...
    async _pushRoundInitiatives(roundIndex=null) {
        let initiatives = {};
        let history     = (this.getFlag("tbsh", "roundHistory") || []).slice();
        ...
        await this.setFlag("tbsh", "roundHistory", history);
    }
}
